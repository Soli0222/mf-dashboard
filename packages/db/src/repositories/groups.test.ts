...
describe("upsertGroup", () => {
  ...
  test("新しいグループをupsertすると古いグループのisCurrentがfalseになる", async () => {
    ...
    const rows = await db.select().from(schema.groups).where(eq(schema.groups.id, "g1"));
    const g1 = rows[0];
  ...
  test("isCurrent=falseのグループをupsertしても他のグループのisCurrentは変わらない", async () => {
    ...
    const rows1 = await db.select().from(schema.groups).where(eq(schema.groups.id, "g1"));
    const g1 = rows1[0];
    const rows2 = await db.select().from(schema.groups).where(eq(schema.groups.id, "g2"));
    const g2 = rows2[0];
...
describe("updateGroupLastScrapedAt", () => {
  ...
  test("updatedAt も更新される", async () => {
    await upsertGroup(db, { id: "g1", name: "テストグループ", isCurrent: true });
    const beforeRows = await db.select().from(schema.groups).where(eq(schema.groups.id, "g1"));
    ...
    const afterRows = await db.select().from(schema.groups).where(eq(schema.groups.id, "g1"));
