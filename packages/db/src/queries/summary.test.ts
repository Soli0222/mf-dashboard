...
describe("buildRegularIncomeSum", () => {
  it("収入のみを合計する", async () => {
    ...
    const result = (await db.select({ total: incomeSum }).from(schema.transactions))[0];
...
describe("buildExpenseSum", () => {
  it("通常の支出のみを合計する（振替は含めない）", async () => {
    ...
    const result = (await db.select({ total: expenseSum }).from(schema.transactions))[0];
